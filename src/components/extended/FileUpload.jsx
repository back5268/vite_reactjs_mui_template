...
const FileUpload = (props) => {
  ...
  const { title = 'Chọn file', files = [], setFiles = () => {}, multiple = true } = props;
          ...
          {files[0] &&
            files.map((f, index) => (
              <Stack key={index} sx={{ maxHeight: 200, maxWidth: 200 }} spacing={1}>
                ...
                {multiple && <Box display={"flex"} justifyContent={"center"}><Button sx={{ p: "4px 0" }} onClick={() => removeFile(f)} type="button" color="error" variant="contained"><ClearIcon /></Button></Box>}
              </Stack>
            ))}
        </Box>
      </Card>
